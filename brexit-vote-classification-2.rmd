...
mod_tree <- tree(fm_all, data=data_train_preped)
plot(mod_tree)
text(mod_tree)
pred_test_tree <- predict(mod_tree, newdata = data_test_preped, type = 'class') 
confusionMatrix(pred_test_tree, data_test_preped$LeaveVote, positive = "1", 
                mode = "prec_recall")
```
...
pred_test_rf <- predict(mod_rf, newdata = data_test_preped)
confusionMatrix(pred_test_rf, data_test_preped$LeaveVote, positive = "1", 
                mode = "prec_recall")
